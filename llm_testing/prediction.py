...
class TokenPredictor:
    def __init__(self, data_path=None, text_input=None, model_name="Qwen/Qwen2.5-0.5B", reduce_tokens=True, first_n_tokens=10000):
        ...
        if data_path:
            data = self._get_data_from_file(data_path)
        else:
            data = text_input
        truncated_data = " ".join(data.split(" ")[:first_n_tokens])

        print(f"Starting tokenization...")
        start_time = time.time()
        self.data_tokens = self.tokenizer.encode(truncated_data, truncation=True, max_length=first_n_tokens)
        assert len(self.data_tokens) == first_n_tokens, f"Tokenization produced {len(self.data_tokens)} tokens, expected {first_n_tokens}."
    ...
    def run_batched_inference(self, prompts):
        ...
        input_ids = torch.tensor(input_ids, device="cuda" if torch.cuda.is_available() else "cpu")
        ...
        last_token_idx = seq_lengths - 1                 # index of real last token

        batch_idx = torch.arange(input_ids.size(0), device=input_ids.device)
        batch_logits = outputs.logits[batch_idx, last_token_idx, :]   # shape: (batch, vocab)
        ...
        return self.tokens_list, batch_probs
